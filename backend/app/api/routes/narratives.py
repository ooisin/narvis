...
from backend.app.crud import get_narrative_by_id, get_narratives
...
def create_narrative(narrative: Narrative, session: SessionDep) -> Any:
    try:
...
def delete_narrative(narrative_id: int, session: SessionDep) -> Any:
    narrative = get_narrative_by_id(session=session, narrative_id=narrative_id)
    if not narrative:
        raise HTTPException(status_code=404, detail="Narrative not found")
    try:
